...
export default function Toolbar() {
  ...
  return (
    <div className="flex w-16 flex-col gap-2 border-r border-slate-800 bg-slate-900 p-2">
      {TOOLS.map((tool) => (
        <div
          key={tool.label}
          draggable
          onDragStart={(e) => onDragStart(e, tool)}
          className={`cursor-grab rounded p-2 text-center text-xs font-medium text-white ${tool.color} hover:opacity-90 active:cursor-grabbing`}
          title={tool.label}
        >
          {tool.label.split(" ").map((w) => w[0]).join("")}
        </div>
      ))}
    </div>
  );
